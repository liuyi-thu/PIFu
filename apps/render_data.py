...
# 垃圾PIFu，毁我青春。必须用三角面片，否则出现迷惑行为。
from lib.renderer.camera import Camera
import numpy as np
from lib.renderer.mesh import load_obj_mesh, compute_tangent, compute_normal, load_obj_mesh_mtl, load_obj_mesh_mtl_new
from lib.renderer.camera import Camera
import os
import cv2
import time
...
def render_prt_ortho(out_path, folder_name, subject_name, shs, rndr, rndr_uv, im_size, angl_step=4, n_light=1,
    ...
    if not os.path.exists(face_prt_file):
        ...
        return
    text_file = os.path.join(folder_name, 'tex', subject_name + '_dif_2k.png') #original: jpg
...
def render_prt_ortho_multimtl(out_path, folder_name, subject_name, shs, rndr, rndr_uv, im_size, angl_step=4, n_light=1,
                              pitch=[0]):
    cam = Camera(width=im_size, height=im_size)
    cam.ortho_ratio = 0.4 * (512 / im_size)
    cam.near = -100
    ...
    #     print('ERROR: dif file does not exist!!', text_file)
    #     return
    vertices, faces, norms, face_normals, uvs, face_uvs, face_data_mat, face_norm_data_mat, face_uv_data_mat, mtl_data = load_obj_mesh_mtl_new(
        mesh_file)
    vmin = vertices.min(0)
    vmax = vertices.max(0)
    up_axis = 1 if (vmax - vmin).argmax() == 1 else 2
    ...
    for mtl in mtl_data:
        print(mtl)
        text_file = os.path.join(folder_name, mtl_data[mtl]['map_Kd'])
        print(text_file)
        texture_image = cv2.imread(text_file)
        texture_image = cv2.cvtColor(texture_image, cv2.COLOR_BGR2RGB)
        # tan, bitan = compute_tangent(vertices, face_data_mat[mtl], face_norm_data_mat[mtl], face_norm_data_mat[mtl],
        #                              face_uv_data_mat[mtl])  # 只用第三个参数。似乎这两个值不很关键
        tan, bitan = compute_tangent(vertices, face_data_mat[mtl], norms, face_norm_data_mat[mtl],
                                                                   face_uv_data_mat[mtl])  # 只用第三个参数。似乎这两个值不很关键
        prt = np.loadtxt(prt_file)
        face_prt = np.load(face_prt_file)
        rndr.set_mesh(vertices, face_data_mat[mtl], norms, face_norm_data_mat[mtl], uvs, face_uv_data_mat[mtl], prt,
                      face_prt, tan, bitan, mat_name=mtl)
        # rndr.set_mesh(vertices, faces, norms, face_normals, uvs, face_uvs, prt, face_prt, tan, bitan)
        #不带face的三项令人起疑，但又不能说啥
        #但是其实并没有多大变化，问题没那么简单了。
        rndr.set_albedo(texture_image, mat_name=mtl)
        rndr_uv.set_mesh(vertices, face_data_mat[mtl], norms, face_norm_data_mat[mtl], uvs, face_uv_data_mat[mtl], prt,
                         face_prt, tan, bitan, mat_name=mtl)
        # rndr_uv.set_mesh(vertices, faces, norms, face_normals, uvs, face_uvs, prt, face_prt, tan, bitan)
        rndr_uv.set_albedo(texture_image, mat_name=mtl)
        pass
...
if __name__ == '__main__':
    ...
    #render_prt_ortho_multimtl(args.out_dir, args.input, subject_name, shs, rndr, rndr_uv, args.size, 1, 1, pitch=[0])
